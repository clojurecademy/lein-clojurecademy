(ns leiningen.clojurecademy.http.client
  "Batteries-included HTTP client."
  (:require [leiningen.clojurecademy.http.conn-mgr :as conn]
            [leiningen.clojurecademy.http.cookies :refer [wrap-cookies]]
            [leiningen.clojurecademy.http.core :as core]
            [leiningen.clojurecademy.http.headers :refer [wrap-header-map]]
            [leiningen.clojurecademy.http.links :refer [wrap-links]]
            [leiningen.clojurecademy.http.util :refer [opt] :as util]
...
(defmacro with-middleware
  ...
  leiningen.clojurecademy.http.client/wrap-url
  leiningen.clojurecademy.http.client/wrap-method

  Unless you really know what you are doing."
  [middleware & body]
  `(let [m# ~middleware]
     (binding [*current-middleware* m#
               leiningen.clojurecademy.http.client/request (reduce #(%2 %1)
                                               leiningen.clojurecademy.http.core/request
                                               m#)]
       ~@body)))
